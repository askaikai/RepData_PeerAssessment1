...
```{r histTotalNumSteps, message=FALSE, warning=FALSE}
...
```
Mean and median number of steps taken per day is `r format(round(table[1], 2), nsmall = 2)` and `r format(round(table[2], 2), nsmall = 2)`, respectively.

## What is the average daily activity pattern?
1. Make a time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis)
```{r avgDailyACtivityPattern, message=FALSE, warning=FALSE, results='hide'}
...
print(table, type="html")
```
Mean and median number of steps taken per day is `r format(round(table[1], 2), nsmall = 2)` and `r format(round(table[2], 2), nsmall = 2)`, respectively. These numbers are slightly different from the original data set, but the difference is very small. Now I'm going to make histograms to see where the difference is coming from.


```{r histOrigVsFilledData, message=FALSE, warning=FALSE}
...
```{r weekdayVsWeekendSteps, message=FALSE, warning=FALSE}
